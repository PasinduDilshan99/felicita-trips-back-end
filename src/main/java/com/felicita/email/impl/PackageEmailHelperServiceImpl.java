...
public class PackageEmailHelperServiceImpl implements PackageEmailHelperService {
    ...
    @Override
    public String buildPackageTypeTerminateSuccessfullSubject(User loggedUser, PackageTypeBasicDetailsResponse packageTypeResponse) {
        return String.format("[Felicita Trips] Package Type Terminated — %s",
                packageTypeResponse.getTypeName() != null ?
                        packageTypeResponse.getTypeName() : "Unknown Type");
    }

    @Override
    public String buildPackageTypeTerminateSuccessfullBody(User loggedUser, PackageTypeBasicDetailsResponse packageTypeResponse) {
        String imagesHtml = buildTerminatePackageTypeImagesHtml(packageTypeResponse.getImages());

        return "<!DOCTYPE html>" +
                "<html lang='en'><head><meta charset='UTF-8'/><meta name='viewport' content='width=device-width,initial-scale=1'/>" +
                "<title>Package Type Terminated</title>" +
                "<style>" +
                "body{margin:0;padding:0;background:#f0f7f7;font-family:Georgia,serif;}" +
                ...
                "<div class='terminate-warning'>" +
                "<span class='warning-icon'>⚠️</span>" +
                "<p><strong>This package type has been terminated</strong> and is no longer available for assignment to packages.</p>" +
                "</div>" +
                ...
                "<div class='info-card'>" +
                "<div class='info-row'><span class='info-label'>Type ID</span><span class='info-value'><span class='reference-id'>#" + packageTypeResponse.getTypeId() + "</span></span></div>" +
                "<div class='info-row'><span class='info-label'>Type Name</span><span class='info-value'><span class='type-icon'>🏷️</span>" + escapeHtml(packageTypeResponse.getTypeName()) + "</span></div>" +
                "<div class='info-row'><span class='info-label'>Description</span><span class='info-value'>" + (packageTypeResponse.getDescription() != null ? escapeHtml(packageTypeResponse.getDescription()) : "—") + "</span></div>" +
                "<div class='info-row'><span class='info-label'>Colors</span><span class='info-value'>" +
                "<span class='color-preview' style='background-color:" + (packageTypeResponse.getColor() != null ? packageTypeResponse.getColor() : "#cccccc") + ";'></span>" +
                "Main: " + (packageTypeResponse.getColor() != null ? packageTypeResponse.getColor() : "—") +
                (packageTypeResponse.getHoverColor() != null ? " | Hover: " + packageTypeResponse.getHoverColor() : "") +
                "</span></div>" +
                "<div class='info-row'><span class='info-label'>Previous Status</span><span class='info-value'>" + buildStatusPill(packageTypeResponse.getStatus()) + "</span></div>" +
                "<div class='info-row'><span class='info-label'>New Status</span><span class='info-value'><span class='status-pill-terminated'>TERMINATED</span></span></div>" +
                "</div>" +

                (imagesHtml.isEmpty() ? "" :
                        "<p class='section-title'>Type Images (" + (packageTypeResponse.getImages() != null ? packageTypeResponse.getImages().size() : 0) + ")</p>" +
                                imagesHtml) +
    ...
    @Override
    public String buildPackageTypeCreateSuccessfullSubject(Long packageTypeId, PackageTypeInsertRequest packageTypeInsertRequest, User loggedUser) {
        return String.format("[Felicita Trips] Package Type Created — %s",
                packageTypeInsertRequest.getTypeName() != null ?
                        packageTypeInsertRequest.getTypeName() : "Unknown Type");
    }

    @Override
    public String buildPackageTypeUpdateSuccessfullSubject(User loggedUser, PackageTypeUpdateRequest packageTypeUpdateRequest) {
        return String.format("[Felicita Trips] Package Type Updated — %s",
                packageTypeUpdateRequest.getTypeName() != null ?
                        packageTypeUpdateRequest.getTypeName() : "Unknown Type");
    }

    @Override
    public String buildPackageTypeUpdateSuccessfullBody(User loggedUser, PackageTypeUpdateRequest packageTypeUpdateRequest, PackageTypeComparisonResult comparisonResult) {
        String basicFieldsHtml = buildPackageTypeBasicFieldsHtml(comparisonResult.getFieldChanges());
        String imagesHtml = buildPackageTypeImageChangesHtml(comparisonResult);
        boolean hasAnyUpdates = comparisonResult != null && comparisonResult.isHasChanges();

        return "<!DOCTYPE html>" +
                "<html lang='en'><head><meta charset='UTF-8'/><meta name='viewport' content='width=device-width,initial-scale=1'/>" +
                "<title>Package Type Updated</title>" +
                "<style>" +
                "body{margin:0;padding:0;background:#f0f7f7;font-family:Georgia,serif;}" +
                ".wrapper{max-width:640px;margin:40px auto;background:#ffffff;border-radius:12px;overflow:hidden;box-shadow:0 4px 24px rgba(0,128,128,0.10);}" +
                ".header{background:linear-gradient(135deg,#0e7c7b 0%,#1a9e9e 50%,#2bbfbf 100%);padding:36px 40px 28px;text-align:center;}" +
                ...
                "<div class='info-card'>" +
                "<div class='info-row'><span class='info-label'>Type ID</span><span class='info-value'><span class='reference-id'>#" + packageTypeUpdateRequest.getTypeId() + "</span></span></div>" +
                "<div class='info-row'><span class='info-label'>Type Name</span><span class='info-value'>" + escapeHtml(packageTypeUpdateRequest.getTypeName()) + "</span></div>" +
                "</div>" +
                ...
                (hasAnyUpdates ? "" : "<div class='no-updates'>No changes were made to this package type.</div>") +
    ...
    @Override
    public String buildPackageScheduleCreateSuccessfullSubject(Long packageScheduleId, PackageScheduleInsertRequest packageScheduleInsertRequest, User loggedUser) {
        return String.format("[Felicita Trips] Package Schedule Created — %s",
                packageScheduleInsertRequest.getPackageScheduleName() != null ?
                        packageScheduleInsertRequest.getPackageScheduleName() : "Unknown Schedule");
    }

    @Override
    public String buildPackageScheduleUpdateSuccessfullSubject(User loggedUser, PackageScheduleUpdateRequest packageScheduleUpdateRequest) {
        return String.format("[Felicita Trips] Package Schedule Updated — %s",
                packageScheduleUpdateRequest.getPackageScheduleName() != null ?
                        packageScheduleUpdateRequest.getPackageScheduleName() : "Unknown Schedule");
    }

    @Override
    public String buildPackageScheduleUpdateSuccessfullBody(User loggedUser, PackageScheduleUpdateRequest packageScheduleUpdateRequest, PcakageScheduleComparisonResult comparisonResult) {
        String basicFieldsHtml = buildPackageScheduleBasicFieldsHtml(comparisonResult.getFieldChanges());
        boolean hasAnyUpdates = comparisonResult != null && comparisonResult.isHasChanges();

        return "<!DOCTYPE html>" +
                "<html lang='en'><head><meta charset='UTF-8'/><meta name='viewport' content='width=device-width,initial-scale=1'/>" +
                "<title>Package Schedule Updated</title>" +
                "<style>" +
                "body{margin:0;padding:0;background:#f0f7f7;font-family:Georgia,serif;}" +
                ".wrapper{max-width:640px;margin:40px auto;background:#ffffff;border-radius:12px;overflow:hidden;box-shadow:0 4px 24px rgba(0,128,128,0.10);}" +
                ".header{background:linear-gradient(135deg,#0e7c7b 0%,#1a9e9e 50%,#2bbfbf 100%);padding:36px 40px 28px;text-align:center;}" +
                ...
                "<div class='header'>" +
                "<img src='https://res.cloudinary.com/dtzrivqye/image/upload/v1775493945/gi5x2y4vwaplhkwchp0p.png' alt='Felicita Trips'/>" +
                "<h1>Package Schedule Updated</h1>" +
                "<p class='tagline'>See More! Feel More! Live More!</p>" +
                "<span class='badge'>&#9998; Update Completed</span>" +
                "</div>" +
                ...
                "<div class='info-card'>" +
                "<div class='info-row'><span class='info-label'>Schedule ID</span><span class='info-value'><span class='reference-id'>#" + packageScheduleUpdateRequest.getPackageScheduleId() + "</span></span></div>" +
                "<div class='info-row'><span class='info-label'>Schedule Name</span><span class='info-value'>" + escapeHtml(packageScheduleUpdateRequest.getPackageScheduleName()) + "</span></div>" +
                "<div class='info-row'><span class='info-label'>Package ID</span><span class='info-value'>#" + (packageScheduleUpdateRequest.getPackageId() != null ? packageScheduleUpdateRequest.getPackageId() : "—") + "</span></div>" +
                "<div class='info-row'><span class='info-label'>Tour Schedule ID</span><span class='info-value'>" +
                (packageScheduleUpdateRequest.getTourScheduleId() != null ? "#" + packageScheduleUpdateRequest.getTourScheduleId() : "—") +
                "</span></div>" +
                "</div>" +
                ...
                (hasAnyUpdates ? "" : "<div class='no-updates'>No changes were made to this package schedule.</div>") +
                        ...
                        "<p class='section-title'>Validation Information</p>" +
                                ...
                                (comparisonResult.getDurationDifference() != null ?
                                        "<div class='validation-item'>" +
                                                ...
                                                "<span class='validation-value'>" + comparisonResult.getDurationDifference() + " days</span>" +
                                                "</div>" : "") +
    ...
    @Override
    public String buildPackageScheduleTerminateSuccessfullSubject(User loggedUser, PacakgeScheduleBasicDetailsResponse packageScheduleResponse) {
        return String.format("[Felicita Trips] Package Schedule Terminated — %s",
                packageScheduleResponse.getPackageScheduleName() != null ?
                        packageScheduleResponse.getPackageScheduleName() : "Unknown Schedule");
    }

    @Override
    public String buildPackageScheduleTerminateSuccessfullBody(User loggedUser, PacakgeScheduleBasicDetailsResponse packageScheduleResponse) {
        return "<!DOCTYPE html>" +
                "<html lang='en'><head><meta charset='UTF-8'/><meta name='viewport' content='width=device-width,initial-scale=1'/>" +
                "<title>Package Schedule Terminated</title>" +
                "<style>" +
                "body{margin:0;padding:0;background:#f0f7f7;font-family:Georgia,serif;}" +
                ".wrapper{max-width:640px;margin:40px auto;background:#ffffff;border-radius:12px;overflow:hidden;box-shadow:0 4px 24px rgba(0,128,128,0.10);}" +
                ".header{background:linear-gradient(135deg,#8B0000 0%,#b22222 50%,#cd5c5c 100%);padding:36px 40px 28px;text-align:center;}" +
                ...
                "<div class='header'>" +
                "<img src='https://res.cloudinary.com/dtzrivqye/image/upload/v1775493945/gi5x2y4vwaplhkwchp0p.png' alt='Felicita Trips'/>" +
                "<h1>Package Schedule Terminated</h1>" +
                "<p class='tagline'>See More! Feel More! Live More!</p>" +
                "<span class='badge'>&#10060; Termination Completed</span>" +
                "</div>" +
                ...
                "<div class='terminate-warning'>" +
                "<span class='warning-icon'>⚠️</span>" +
                "<p><strong>This package schedule has been terminated</strong> and is no longer available for booking or display.</p>" +
                "</div>" +
                ...
                "<div class='info-card'>" +
                "<div class='info-row'><span class='info-label'>Schedule ID</span><span class='info-value'><span class='reference-id'>#" + packageScheduleResponse.getPackageScheduleId() + "</span></span></div>" +
                "<div class='info-row'><span class='info-label'>Schedule Name</span><span class='info-value'><span class='schedule-icon'>📅</span>" + escapeHtml(packageScheduleResponse.getPackageScheduleName()) + "</span></div>" +
                "<div class='info-row'><span class='info-label'>Package</span><span class='info-value'>" +
                (packageScheduleResponse.getPackageName() != null ? escapeHtml(packageScheduleResponse.getPackageName()) : "#" + packageScheduleResponse.getPackageId()) +
                "</span></div>" +
                "<div class='info-row'><span class='info-label'>Tour Schedule</span><span class='info-value'>" +
                (packageScheduleResponse.getTourScheduleName() != null ? escapeHtml(packageScheduleResponse.getTourScheduleName()) :
                        (packageScheduleResponse.getTourScheduleId() != null ? "#" + packageScheduleResponse.getTourScheduleId() : "—")) +
                "</span></div>" +
                "<div class='info-row'><span class='info-label'>Schedule Period</span><span class='info-value'><span class='date-range'>" +
                (packageScheduleResponse.getAssumeStartDate() != null ? formatDate(packageScheduleResponse.getAssumeStartDate()) : "—") +
                "</span> → <span class='date-range'>" +
                (packageScheduleResponse.getAssumeEndDate() != null ? formatDate(packageScheduleResponse.getAssumeEndDate()) : "—") +
                "</span></span></div>" +
                "<div class='info-row'><span class='info-label'>Duration Range</span><span class='info-value'><span class='duration-range'>" +
                (packageScheduleResponse.getDurationStart() != null ? packageScheduleResponse.getDurationStart() : "—") +
                " - " +
                (packageScheduleResponse.getDurationEnd() != null ? packageScheduleResponse.getDurationEnd() : "—") +
                " days</span></span></div>" +
                "<div class='info-row'><span class='info-label'>Previous Status</span><span class='info-value'>" + buildStatusPill(packageScheduleResponse.getStatus()) + "</span></div>" +
                "<div class='info-row'><span class='info-label'>New Status</span><span class='info-value'><span class='status-pill-terminated'>TERMINATED</span></span></div>" +
                "<div class='info-row'><span class='info-label'>Description</span><span class='info-value'>" +
                (packageScheduleResponse.getDescription() != null ? escapeHtml(packageScheduleResponse.getDescription()) : "—") +
                "</span></div>" +
                (packageScheduleResponse.getSpecialNote() != null && !packageScheduleResponse.getSpecialNote().isEmpty() ?
                        "<div class='special-notes-box'><p><strong>📝 Special Notes:</strong> " + escapeHtml(packageScheduleResponse.getSpecialNote()) + "</p></div>" : "") +
                "</div>" +
                ...
                (packageScheduleResponse.getCreatedBy() != null ? "User #" + packageScheduleResponse.getCreatedBy() : "—") +
                "</span></div>" +
                "<div class='info-metadata-row'><span class='info-metadata-label'>Created At</span><span class='info-metadata-value'>" +
                (packageScheduleResponse.getCreatedAt() != null ? formatDateTime(packageScheduleResponse.getCreatedAt()) : "—") +
                "</span></div>" +
                "<div class='info-metadata-row'><span class='info-metadata-label'>Last Updated By</span><span class='info-metadata-value'>" +
                (packageScheduleResponse.getUpdatedBy() != null ? "User #" + packageScheduleResponse.getUpdatedBy() : "—") +
                "</span></div>" +
                "<div class='info-metadata-row'><span class='info-metadata-label'>Last Updated At</span><span class='info-metadata-value'>" +
                (packageScheduleResponse.getUpdatedAt() != null ? formatDateTime(packageScheduleResponse.getUpdatedAt()) : "—") +
                "</span></div>" +
                "</div>" +
    ...
    @Override
    public String buildPackageScheduleCreateSuccessfullBody(Long packageScheduleId, PackageScheduleInsertRequest packageScheduleInsertRequest, User loggedUser) {
        return "<!DOCTYPE html>" +
                "<html lang='en'><head><meta charset='UTF-8'/><meta name='viewport' content='width=device-width,initial-scale=1'/>" +
                "<title>Package Schedule Created</title>" +
                "<style>" +
                "body{margin:0;padding:0;background:#f0f7f7;font-family:Georgia,serif;}" +
                ".wrapper{max-width:640px;margin:40px auto;background:#ffffff;border-radius:12px;overflow:hidden;box-shadow:0 4px 24px rgba(0,128,128,0.10);}" +
                ".header{background:linear-gradient(135deg,#0e7c7b 0%,#1a9e9e 50%,#2bbfbf 100%);padding:36px 40px 28px;text-align:center;}" +
                ...
                "<div class='header'>" +
                "<img src='https://res.cloudinary.com/dtzrivqye/image/upload/v1775493945/gi5x2y4vwaplhkwchp0p.png' alt='Felicita Trips'/>" +
                "<h1>Package Schedule Created</h1>" +
                "<p class='tagline'>See More! Feel More! Live More!</p>" +
                ...
                "<p class='section-title'>Schedule Details</p>" +
                "<div class='info-card'>" +
                "<div class='info-row'><span class='info-label'>Schedule ID</span><span class='info-value'><span class='reference-id'>#" + packageScheduleId + "</span></span></div>" +
                "<div class='info-row'><span class='info-label'>Schedule Name</span><span class='info-value'><span class='schedule-icon'>📅</span>" + escapeHtml(packageScheduleInsertRequest.getPackageScheduleName()) + "</span></div>" +
                "<div class='info-row'><span class='info-label'>Package ID</span><span class='info-value'>#" + (packageScheduleInsertRequest.getPackageId() != null ? packageScheduleInsertRequest.getPackageId() : "—") + "</span></div>" +
                "<div class='info-row'><span class='info-label'>Schedule Period</span><span class='info-value'><span class='date-range'>" +
                (packageScheduleInsertRequest.getAssumeStartDate() != null ? formatDate(packageScheduleInsertRequest.getAssumeStartDate()) : "—") +
                "</span> → <span class='date-range'>" +
                (packageScheduleInsertRequest.getAssumeEndDate() != null ? formatDate(packageScheduleInsertRequest.getAssumeEndDate()) : "—") +
                "</span></span></div>" +
                "<div class='info-row'><span class='info-label'>Duration Range</span><span class='info-value'><span class='duration-range'>" +
                (packageScheduleInsertRequest.getDurationStart() != null ? packageScheduleInsertRequest.getDurationStart() : "—") +
                " - " +
                (packageScheduleInsertRequest.getDurationEnd() != null ? packageScheduleInsertRequest.getDurationEnd() : "—") +
                " days</span></span></div>" +
                "<div class='info-row'><span class='info-label'>Description</span><span class='info-value'>" +
                (packageScheduleInsertRequest.getDescription() != null ? escapeHtml(packageScheduleInsertRequest.getDescription()) : "—") +
                "</span></div>" +
                "<div class='info-row'><span class='info-label'>Tour Schedule ID</span><span class='info-value'>" +
                (packageScheduleInsertRequest.getTourScheduleId() != null ? "#" + packageScheduleInsertRequest.getTourScheduleId() : "—") +
                "</span></div>" +
                "<div class='info-row'><span class='info-label'>Status</span><span class='info-value'>" + buildStatusPill(packageScheduleInsertRequest.getStatus()) + "</span></div>" +
                (packageScheduleInsertRequest.getSpecialNote() != null && !packageScheduleInsertRequest.getSpecialNote().isEmpty() ?
                        "<div class='special-notes-box'><p><strong>📝 Special Notes:</strong> " + escapeHtml(packageScheduleInsertRequest.getSpecialNote()) + "</p></div>" : "") +
                "</div>" +
    ...
    @Override
    public String buildPackageTypeCreateSuccessfullBody(Long packageTypeId, PackageTypeInsertRequest packageTypeInsertRequest, User loggedUser) {
        String imagesHtml = buildCreatePackageTypeImagesHtml(packageTypeInsertRequest.getImages());

        return "<!DOCTYPE html>" +
                "<html lang='en'><head><meta charset='UTF-8'/><meta name='viewport' content='width=device-width,initial-scale=1'/>" +
                "<title>Package Type Created</title>" +
                "<style>" +
                "body{margin:0;padding:0;background:#f0f7f7;font-family:Georgia,serif;}" +
                ".wrapper{max-width:640px;margin:40px auto;background:#ffffff;border-radius:12px;overflow:hidden;box-shadow:0 4px 24px rgba(0,128,128,0.10);}" +
                ".header{background:linear-gradient(135deg,#0e7c7b 0%,#1a9e9e 50%,#2bbfbf 100%);padding:36px 40px 28px;text-align:center;}" +
                ...
                "<p class='section-title'>Type Details</p>" +
                "<div class='info-card'>" +
                "<div class='info-row'><span class='info-label'>Type ID</span><span class='info-value'><span class='reference-id'>#" + packageTypeId + "</span></span></div>" +
                "<div class='info-row'><span class='info-label'>Type Name</span><span class='info-value'><span class='type-icon'>🏷️</span>" + escapeHtml(packageTypeInsertRequest.getTypeName()) + "</span></div>" +
                "<div class='info-row'><span class='info-label'>Description</span><span class='info-value'>" + (packageTypeInsertRequest.getDescription() != null ? escapeHtml(packageTypeInsertRequest.getDescription()) : "—") + "</span></div>" +
                "<div class='info-row'><span class='info-label'>Colors</span><span class='info-value'>" +
                "<span class='color-preview' style='background-color:" + (packageTypeInsertRequest.getColor() != null ? packageTypeInsertRequest.getColor() : "#cccccc") + ";'></span>" +
                "Main: " + (packageTypeInsertRequest.getColor() != null ? packageTypeInsertRequest.getColor() : "—") +
                (packageTypeInsertRequest.getHoverColor() != null ? " | Hover: " + packageTypeInsertRequest.getHoverColor() : "") +
                "</span></div>" +
                "<div class='info-row'><span class='info-label'>Status</span><span class='info-value'>" + buildStatusPill(packageTypeInsertRequest.getStatus()) + "</span></div>" +
                "</div>" +

                (imagesHtml.isEmpty() ? "" :
                        "<p class='section-title'>Images (" + (packageTypeInsertRequest.getImages() != null ? packageTypeInsertRequest.getImages().size() : 0) + ")</p>" +
                                imagesHtml) +
    ...
    private String buildTerminatePackageTypeImagesHtml(List<PackageTypeImageResponse> images) {
        if (images == null || images.isEmpty()) return "";
        StringBuilder sb = new StringBuilder();
        sb.append("<table class='info-table'><thead><tr><th>#</th><th>Name</th><th>Description</th><th>Preview</th></tr></thead><tbody>");
        int i = 1;
        for (PackageTypeImageResponse img : images) {
            sb.append("<tr>")
    ...
    private String buildPackageScheduleBasicFieldsHtml(List<PcakageScheduleComparisonResult.FieldChange> fieldChanges) {
        if (fieldChanges == null || fieldChanges.isEmpty()) return "";
        StringBuilder sb = new StringBuilder();
        for (PcakageScheduleComparisonResult.FieldChange field : fieldChanges) {
    ...
    private String buildCreatePackageTypeImagesHtml(List<PackageTypeImageInsertRequest> images) {
        if (images == null || images.isEmpty()) return "";
        StringBuilder sb = new StringBuilder();
        sb.append("<table class='info-table'><thead><tr><th>#</th><th>Name</th><th>Description</th><th>Status</th><th>Preview</th></tr></thead><tbody>");
        int i = 1;
        for (PackageTypeImageInsertRequest img : images) {
            sb.append("<tr>")
                    .append("<td>").append(i++).append("</td>")
                    .append("<td>").append(escapeHtml(img.getName())).append("</td>")
                    .append("<td>").append(img.getDescription() != null ? escapeHtml(img.getDescription()) : "—").append("</td>")
    ...
    private String buildPackageTypeBasicFieldsHtml(List<PackageTypeComparisonResult.FieldChange> fieldChanges) {
        if (fieldChanges == null || fieldChanges.isEmpty()) return "";
        StringBuilder sb = new StringBuilder();
        for (PackageTypeComparisonResult.FieldChange field : fieldChanges) {
            String displayOldValue = formatValueForDisplay(field.getOldValue());
            String displayNewValue = formatValueForDisplay(field.getNewValue());
    ...
    private String buildPackageTypeImageChangesHtml(PackageTypeComparisonResult comparisonResult) {
        StringBuilder sb = new StringBuilder();
        ...
        if (comparisonResult.getImagesToAdd() != null && !comparisonResult.getImagesToAdd().isEmpty()) {
            ...
            for (PackageTypeComparisonResult.ImageChange img : comparisonResult.getImagesToAdd()) {
                sb.append("<tr>")
                        .append("<td>").append(escapeHtml(img.getName())).append("</td>")
                        .append("<td>").append(img.getDescription() != null ? escapeHtml(img.getDescription()) : "—").append("</td>")
                        .append("<td>").append(buildStatusPill(img.getStatus())).append("</td>")
        ...
        if (comparisonResult.getImagesToUpdate() != null && !comparisonResult.getImagesToUpdate().isEmpty()) {
            ...
            for (PackageTypeComparisonResult.ImageUpdateChange img : comparisonResult.getImagesToUpdate()) {
                // Name change
